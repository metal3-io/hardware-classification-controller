...
func (hcReconciler *HardwareClassificationReconciler) Reconcile(req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	hccLog := hcReconciler.Log.WithName(HWControllerName).WithValues("metal3-hardwareclassification", req.NamespacedName)
	...
	defer func() {
		...
		if err := patchHelper.Patch(ctx, hardwareClassification); err != nil {
			hccLog.Error(err, "Failed to Patch HardwareClassification")
		}
	}()
	...
	hccLog.Info("Expected Hardware Configuration", "Profile", extractedProfile)

	// Get the new hardware classification manager
	hcManager := hcmanager.NewHardwareClassificationManager(hcReconciler.Client, hccLog)
	...
	if ErrValidation != nil {
		...
		hccLog.Error(ErrValidation, ErrValidation.Error())
		return ctrl.Result{}, nil
	}
	...
	hccLog.Info("Validated Hardware Details", "HardwareDetails", validatedHardwareDetails)
	...
	hccLog.Info("Filtered Bare metal hosts", "ValidHosts", validHosts)
	...
	if len(updateLabelError) > 0 {
		...
		hccLog.Error(nil, hwcc.UpdateLabelError)
		...
		hccLog.Info("Updated profile status", "ProfileMatchStatus", hwcc.ProfileMatchStatusMatched)
	} else {
		hcmanager.SetStatus(hardwareClassification, hwcc.ProfileMatchStatusUnMatched,
			hwcc.Empty, hwcc.NOError)
		hccLog.Info("Updated profile status", "ProfileMatchStatus", hwcc.ProfileMatchStatusUnMatched)
	}
